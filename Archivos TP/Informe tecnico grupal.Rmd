...
### Variable categórica medida en escala nominal
 ...
 - Respuestas: Prestado / lo alquilan / es propio sin títulos formales / propio con algún comprobante de tenencia / es ocupado / otro (en este caso especificar)
...
### Variable categórica de respuesta múltiple
  ...
  Se propone analizar la presencia de plagas en la vivienda y sus alrededores. Para ello, se toma como variable la respuesta a la pregunta "10.6.2. ¿Hay plagas en su vivienda y en los alrededores de la misma? ¿Cuáles?", cuyas opciones incluyen Cucarachas, Mosquitos y/o Ratas. 

```{r}
df_cuca <- df %>% select(ORDEN, PROVINCIA, BARRIO, id_10.6.2_1, id_10.6.2_2, id_10.6.2_3)
#df_cuca
```
...
df_cuca %>% 
  ...
  filter(!is.na(id_10.6.2_1)) %>%
  arrange(desc(CUCARACHAS)) %>%
  select(BARRIO, CUCARACHAS)
```
...
```{r}
df_inte <- df %>% select(ORDEN, PROVINCIA, BARRIO, id_1.4_1)
```
